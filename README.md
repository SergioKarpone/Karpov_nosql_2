 ...
 *Примітка.* Файл `embeddings.npy` має розмір 29.3 Mb, при обмеженні GitHub у 25 Mb
...
`cos(q, d) = q · d`. Дві метрики рахують буквально одне й те саме число, отже й 
сортування ідентичне.
...
*Примітка.* Вивід показує year 2007 -> 10000: усі 10 000 статей за 2007 рік. Це 
тому, що 01_prepare_data.py бере перші 10 000 рядків снапшоту, а вони всі з 
arXiv ID 0704.xxxx (фізика/астро). 

Наслідки:
- Фільтр A в 04_search.py («останні 5 років», year ≥ 2021) повертає 0 результатів
  і на скріні між блоками 2A та 2B порожньо. Це не баг, а наслідок датасету.
- Запити про BERT / CNN LeCun / емоції-з-тексту не мають по-справжньому 
  релевантних статей, бо таких робіт (2018+ NLP/CV) у зрізі 2007 року просто 
  немає - тому такі результати.
...
BM25 - лексичний: рахує збіг токенів із поправкою на частоту й довжину документа. 
Він виграє там, де запит = точний рідкісний токен: абревіатури (`BERT`), назви 
методів (`fine-tuning`), прізвища авторів (`Yann LeCun`) - у наших прогонах 
саме ці запити BM25 ранжував найточніше. Семантичний (векторний) пошук оперує 
змістом: запит `making computers understand human emotions from text` не має 
спільних слів зі статтями про *sentiment analysis*, але вектор їх знаходить -
